...
void enableRawMode() {
  ...
  */
  ...
  can return. We set it to 0 so that read() returns as soon as there is any
  ...
  second, or 100 milliseconds. If read() times out, it will return 0, which
  makes sense because its usual return value is the number of bytes read.
  */
  raw.c_cc[VMIN] = 0;
  raw.c_cc[VTIME] = 1;
...
// ABUF_INIT constant which represents an empty buffer.
// This acts as a constructor for our abuf type.
#define ABUF_INIT \
  { NULL, 0 }
